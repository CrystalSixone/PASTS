...
class MlmDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        output['traj_view_img_fts'] = [torch.from_numpy(x) for x in inputs['traj_view_img_fts']] # [len,36,768]
        if 'traj_obj_img_fts' in inputs:
            output['traj_obj_img_fts'] = [torch.from_numpy(x) for x in inputs['traj_obj_img_fts']]  # [len,obj_len,2048]
