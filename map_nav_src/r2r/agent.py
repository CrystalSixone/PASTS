...
class GMapNavAgent(Seq2SeqAgent):
    ...
    def rollout(self, train_ml=None, train_rl=False, reset=True, speaker=None):
        ...
        if speaker is not None:        
            noise = self.vln_bert.drop_env(torch.ones(self.args.feature_size).cuda())
            ...
            for i in range(len(insts)):
                length = len(insts[i])
                if length < self.args.maxInput:
                    temp = np.hstack((insts[i],np.array([0]*(self.args.maxInput-length))))
                else:
                    temp = insts[i][:self.args.maxInput]
                new_insts.append(temp)
            new_insts = np.array(new_insts)
            ...
            for i, (datum, inst) in enumerate(zip(batch, new_insts)):
                ...
                padded_instr_tokens, num_words = self.pad_instr_tokens(instr_tokens, self.args.maxInput)
                datum['instr_encoding'] = self.tok.convert_tokens_to_ids(padded_instr_tokens)
