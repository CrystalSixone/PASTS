...
class PASTSEncoder(nn.Module):
    def __init__(self, feature_size, hidden_size,feat_drop_ratio=args.featdropout):
        super(PASTSEncoder, self).__init__()
        self.hidden_size = hidden_size
        self.feature_size = feature_size

        self.pos_feature = PositionalEncoding(feature_size) 
...
class PASTSDecoder(nn.Module):
    ...
    def forward(self, dec_inputs, enc_outputs, ctx_mask=None):
...
class ProgressEstimator(nn.Module):
    def __init__(self, in_dim, hid_dim, out_dim=1, dropout=args.speaker_dropout):
        super(ProgressEstimator, self).__init__()
        layers = [
            nn.Linear(in_dim, hid_dim).cuda(),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(hid_dim, out_dim).cuda()
        ]
...
class PASTS(nn.Module):
    def __init__(self, feature_size, hidden_size, word_size, tgt_vocab_size, padding_idx=0):
        super(PASTS, self).__init__()
        self.encoder = PASTSEncoder(feature_size, hidden_size).cuda()
        self.decoder = PASTSDecoder(tgt_vocab_size, word_size, hidden_size, padding_idx=padding_idx).cuda()
        self.projection = nn.Linear(word_size, tgt_vocab_size, bias=False).cuda()
        self.progress_projection = ProgressEstimator(word_size,args.proj_hidden).cuda()
        self.dropout = nn.Dropout(args.speaker_dropout)
    ...
    def forward(self, action_embeddings,world_state_embeddings, dec_inputs,ctx_mask=None,act_type_ids=None,feat_type_ids=None,already_dropfeat=False):
        _, enc_outputs, enc_self_attns = self.encoder(action_embeddings,world_state_embeddings,
                                                               already_dropfeat=already_dropfeat)
        dec_outputs, dec_self_attns, dec_enc_attns = self.decoder(dec_inputs, enc_outputs,ctx_mask=ctx_mask) 
        if args.use_drop:
            dec_outputs = self.dropout(dec_outputs) 
        dec_logits = self.projection(dec_outputs) 
        progress_prediction = self.progress_projection(dec_outputs)
        return dec_logits, progress_prediction, enc_self_attns, dec_self_attns, dec_enc_attns 
